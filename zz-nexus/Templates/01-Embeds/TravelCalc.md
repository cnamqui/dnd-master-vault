...
| Miles To Travel:  `VIEW[{TravelDistanceKM}*0.62][:TravelDistance]`  |
| KMs To Travel:  `INPUT[number:TravelDistanceKM]`  |
...
| **Hours Travel 🕓:** `VIEW[round(({TravelDistance} * ({varMins}/(({BaseSpeed} / ({ExhaustionLevel} > 1 ? 2 : 1) + ({Encumbered} ? -10 : 0) + ({HorseshoesofSpeed} ? 30 : 0) + {AdditionalBonus}) / 10) * {SpeedMultiplier})) / 60, 1)]`  |
...
> **Travel Calc:** `VIEW[({varMins}/(({BaseSpeed} / ({ExhaustionLevel} > 1 ? 2 : 1) + ({Encumbered} ? -10 : 0) + ({HorseshoesofSpeed} ? 30 : 0) + {AdditionalBonus}) / 10) * {SpeedMultiplier})][math:TravelCalc]`
